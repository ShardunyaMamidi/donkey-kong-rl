...
- [ ] **1. Preprocessing pipeline** - grayscale, resize/downsample (e.g. 84x84),
      normalize pixel values, stack last N frames (e.g. 4) for motion info
- [ ] **2. Environment wrappers** - frame-skipping w/ max-pooling over last 2
...
- [ ] **3. Replace Q-table with a neural network (CNN)** - takes stacked frames
      as input, outputs one Q-value per action (replaces `self.qtable`)
- [ ] **4. Rework `choose_action`** - forward pass through network instead of
      `q_table[state, :]`; keep epsilon-greedy logic
- [ ] **5. Add a replay buffer (experience replay)** - store
      `(state, action, reward, next_state, done)` transitions, train on random
      mini-batches instead of updating every step
- [ ] **6. Add a target network** - slowly-updated copy of the network used to
      compute target Q-values, for training stability
- [ ] **7. Define loss + optimizer** - MSE/Huber loss between predicted and
      target Q, backprop via Adam (replaces the tabular update rule)
- [ ] **8. Restructure training loop** - reset -> act -> step -> push to buffer
      -> periodically sample batch + gradient step -> periodically sync target
      network
- [ ] **9. Epsilon decay schedule** - decay from ~1.0 to ~0.01-0.1 over
      training instead of a fixed epsilon
- [ ] **10. New hyperparameters** - replay buffer size, batch size, target
      network update frequency, learning rate (~1e-4), frame-stack size,
      buffer warm-up period
- [ ] **11. Pick deep learning framework** - PyTorch or TensorFlow (numpy
